...
class ExchangeManager
{
    ...
    /**
     * Getting the order book from the Kraken exchange
     *
     * @param string $pair The trading pair
     * @param int $limit The number of records
     * @return array The order book
     */
    private function getOrderBookFromKraken(string $pair, int $limit): array
    {
        try {
            ...
            if ($response === false) {
                throw new Exception("Failed to get the response from the API");
            }
            
            // Decoding the response
            $data = json_decode($response, true);
            if (!$data || !isset($data['bids']) || !isset($data['asks'])) {
            ...
            $this->logger->error("Error getting the order book from Kraken for pair {$pair}: " . $e->getMessage());
            
            // If an error occurred, return a simulated order book
            $bids = [];
            $asks = [];
            
            // Getting the current market price (simulation)
            $marketPrice = $this->getMarketPrice($pair);
            
            // Generating bids (below the market price)
            for ($i = 0; $i < $limit; $i++) {
                $price = $marketPrice * (1 - 0.01 * ($i + 1) / 10);
                $amount = mt_rand(1, 10) / 10;
                $bids[] = [$price, $amount];
            }
            
            // Generating asks (above the market price)
            for ($i = 0; $i < $limit; $i++) {
                $price = $marketPrice * (1 + 0.01 * ($i + 1) / 10);
                $amount = mt_rand(1, 10) / 10;
                $asks[] = [$price, $amount];
            ...
            // Sorting the bids by descending price
            usort($bids, function($a, $b) {
                return $b[0] <=> $a[0];
            });
            
            // Sorting the asks by ascending price
    ...
    /**
     * Getting the order book from the Binance exchange
     *
     * @param string $pair The trading pair
     * @param int $limit The number of records
     * @return array The order book
     */
    private function getOrderBookFromBinance(string $pair, int $limit): array
    {
        try {
            ...
            $this->logger->error("Error getting the order book from Binance for pair {$pair}: " . $e->getMessage());
